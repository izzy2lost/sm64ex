...
int saturn_rom_status(fs::path extract_dest, std::vector<std::string>* todo, int type) {
    ...
    if (needs_rom && !fs::exists(gCLIOpts.RomPath)) {
#ifdef FILE_PICKER
        open_file_picker();
#endif
        if (!fs::exists(gCLIOpts.RomPath)) return ROM_MISSING;
    }
...
int saturn_extract_rom(int type) {
    ...
    if (status == ROM_MISSING) {
        pfd::message("Missing ROM", "baserom.us.z64 has not been extracted. \n\nPlease select your unmodified US Super Mario 64 z64 ROM by placing \"baserom.us.z64\" in the current working directory, using the --rom argument, or using the GTK file picker.", pfd::choice::ok);
        return ROM_MISSING;
    }
    if (status == ROM_INVALID) {
        pfd::message("Invalid ROM", "Couldn't verify baserom.us.z64\n\nThe file may be corrupted, extended, or from the wrong region. Use an unmodified US version of SM64", pfd::choice::ok);
        return ROM_INVALID;
    }
